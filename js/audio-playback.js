/**
 * audio-playback.js (v3) — 내가 말한 음성 재생 UI
 * 마이크 접근은 main.js에서 처리. 여기는 재생 UI만 담당.
 */
(function () {
  "use strict";
  ...
  function togglePlayback() {
    ...
    playbackAudio.ontimeupdate = function () {
      if (playbackAudio.duration && isFinite(playbackAudio.duration)) {
        ...
        document.getElementById("playbackTime").textContent = "0:" + (sec < 10 ? "0" + sec : sec);
      }
    };
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", injectPlaybackUI);
  } else {
    injectPlaybackUI();
  }
})();
